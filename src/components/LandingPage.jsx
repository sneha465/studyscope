...
import { Sparkles, ArrowRight, Brain, Flame, Target, BarChart3 } from "lucide-react";
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 overflow-x-hidden">
      ...
      <main className="max-w-7xl mx-auto px-6 pt-24 pb-32">
        ...
        >
          ...
          >
            <Brain className="w-4 h-4" /> Daily Knowledge Tracking
          </motion.div>
          ...
          >
            Take a daily 10-question quiz, track your knowledge score, build streaks, and master topics over time.
          </motion.p>
                  ...
                  <div className="p-6 bg-slate-900 rounded-2xl border border-slate-800 flex flex-col justify-between">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {[...Array(24)].map((_, i) => (
                        <div
                          key={i}
                          className={`w-3 h-3 rounded-sm ${i % 4 === 0 ? "bg-purple-500" : "bg-slate-800"}`}
                        />
                      ))}
                    </div>
                  </div>
      ...
      <footer className="py-20 border-t border-slate-900 text-center text-slate-500 text-sm">
        <p>© 2026 StudyScope. Built for lifelong learners.</p>
      </footer>
    </div>
  );
}
